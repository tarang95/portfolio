...
function checkLocationNValidation() {
	//allow to use location
	if (navigator && navigator.geolocation)
    {
        navigator.geolocation.getCurrentPosition(this.showLogCheck, errorFunction);
    ...
    {
        navigator.geolocation.getCurrentPosition(showLogCheck, errorFunction);
...
function sendLogEntries() {
	...
	if(!confirm("Effect of deleting all logs?")) {
		return;
	}
...
function showLogCheck(position) 
{
	//saving long and latitude
    latitude = position.coords.latitude;
    longitude = position.coords.longitude;
    //alert('Your latitude is :'+latitude+' and longitude is '+longitude);
	// Get the entire details with validation
	var id = $("#input_id").val();
	if(id == ""){
	...
	// saving date and time in variable
	var today = new Date();
...
{
    alert('Error location not allowed to access!');
}